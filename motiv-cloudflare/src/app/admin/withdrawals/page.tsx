...
import { ArrowLeft, Check, X, Banknote } from "lucide-react";
...
export default function AdminWithdrawals() {
  ...
  async function fetchPayments() {
    ...
    if (res.ok) {
      ...
      setPayments((data.payments || []).filter((p: any) => p.type === "withdrawal"));
    }
    setLoading(false);
  }
        ...
        <div className="space-y-3">
          {filtered.length === 0 && (
            <div className="text-center py-20 text-stone-500">
              <Banknote className="w-10 h-10 mx-auto mb-3 text-stone-600" />
            ...
            <div key={p.id} className="bg-stone-900 rounded-2xl border border-stone-800 p-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-stone-500">{p.user_email} · {formatDate(p.created_at)}</p>
                </div>
                <div className="text-right">
                  <p className="text-xl font-black text-emerald-400">{formatCurrency(p.amount)}</p>
              ...
              {p.status === "pending" && (
                <div className="flex gap-2">
                  <button onClick={() => updatePayment(p.id, "approved", "Refund processed via UPI")}
                    ...
                    <Check className="w-3 h-3" /> Refund
                  </button>
                  <button onClick={() => updatePayment(p.id, "rejected", "Refund denied")}
